...
public class BoardDAO {
	...
	public void insertBoard(BoardBean boardbean) {
		...
		try {
			con=getConnection();
			sql = "select max(num) from f_board";
	...
	public List<BoardBean> getBoardList(int startRow, int pageSize){
		...
		try {
			con = getConnection();
			sql = "select * from f_board order by re_ref desc, re_seq asc limit ?,?";
			pstmt = con.prepareStatement(sql);
	...
	public void updateReadCount(int num) {
		...
		try {
			con=getConnection();
			sql = "update f_board set readcount = readcount + 1 where num = ?";
			pstmt=con.prepareStatement(sql);
			pstmt.setInt(1, num);
...
public BoardBean getBoardInfo(int num) {
		...
		try {
			...
			sql = "select * from f_board where num=?";
	...
	public int updateBoard(BoardBean boardbean) {
		...
		try {
			con=getConnection();
			sql="select passwd from f_board where num=?";
			pstmt=con.prepareStatement(sql);
			...
			if(rs.next()) {
				if(boardbean.getPasswd().equals(rs.getString("passwd"))) {
					check=1;
					sql = "update f_board set name=?, subject=?, content=? where num=?";
					pstmt=con.prepareStatement(sql);
	...
	public void reInsertBoard(BoardBean boardbean) {
		...
		try {
			...
			sql="update f_board set re_seq=re_seq+1 where re_ref=? and re_seq>?";
			pstmt=con.prepareStatement(sql);
			pstmt.setInt(1, boardbean.getRe_ref());
			pstmt.setInt(2, boardbean.getRe_seq());
			pstmt.executeUpdate();
			
			sql="insert into f_board(num,name,passwd,subject,content,re_ref,re_lev,re_seq,readcount,date) values(?,?,?,?,?,?,?,?,?,?)";
			pstmt=con.prepareStatement(sql);
			pstmt.setInt(1, num);
			pstmt.setString(2, boardbean.getName());
			pstmt.setString(3, boardbean.getPasswd());
	...
	public int getCount(String f_search) {
		Connection con =null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql ="";
		...
		try {
			con=getConnection();
			sql = "select count(*) from f_board where subject like ?";
			pstmt=con.prepareStatement(sql);
			pstmt.setString(1, "%"+f_search+"%");
	...
	public List<BoardBean> getBoardList(int startRow, int pageSize, String f_search){
		List<BoardBean> boardList = new ArrayList<BoardBean>();
		Connection con =null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
			con=getConnection();
			sql="select * from f_board where subject like ? order by re_ref desc, re_seq asc limit ?,?";
			pstmt=con.prepareStatement(sql);
			pstmt.setString(1, "%"+f_search+"%");
	...
	public void deleteBoard(int num) {
		...
		try {
			con=getConnection();
			sql="delete from f_board where num = ?";
			pstmt=con.prepareStatement(sql);
			pstmt.setInt(1, num);
			pstmt.executeUpdate();
		} catch (Exception e) {
	...
	public void insertComment(CommentBean cmtbean) {
		...
		try {
			con=getConnection();
			sql="insert into f_comment values(?,?,?,?,now())";
			pstmt=con.prepareStatement(sql);
	...
	public void deleteComment(int num, int cnum) {
		...
		try {
			con = getConnection();
			sql = "delete from f_comment where num = ? and cnum=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			pstmt.setInt(2, cnum);
			pstmt.executeUpdate();
...
public ArrayList<CommentBean> getComment(int num){
		...
		try {
			con=getConnection();
			String sql = "SELECT * FROM f_comment WHERE num = ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			rs = pstmt.executeQuery();
