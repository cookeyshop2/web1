...
public class BoardDAO {
	...
	public void insertBoard(BoardBean boardbean) {
		...
		try {
			...
			sql="insert into n_board "
					+ "(num,name,passwd,subject,"
					+ "content, readcount, re_ref, re_lev, re_seq, date)"
					+ "values(?,?,?,?,?,?,?,?,?,now())";
			pstmt=con.prepareStatement(sql);
			pstmt.setInt(1, num);
	...
	public List<BoardBean> getBoardList(int startRow, int pageSize){
		...
		try {
			con = getConnection();
			sql = "select * from n_board order by re_ref desc, re_seq asc limit ?,?";
			pstmt = con.prepareStatement(sql);
...
public BoardBean getBoardInfo(int num) {
	...
	public int updateBoard(BoardBean boardbean) {
		...
		try {
			con=getConnection();
			sql="select passwd from n_board where num=?";
			pstmt=con.prepareStatement(sql);
			...
			if(rs.next()) {
				if(boardbean.getPasswd().equals(rs.getString("passwd"))) {
					check=1;
					sql = "update n_board set name=?, subject=?, content=? where num=?";
					pstmt=con.prepareStatement(sql);
	...
	public void reInsertBoard(BoardBean boardbean) {
		...
		try {
			...
			sql="update n_board set re_seq=re_seq+1 where re_ref=? and re_seq>?";
			pstmt=con.prepareStatement(sql);
			pstmt.setInt(1, boardbean.getRe_ref());
			pstmt.setInt(2, boardbean.getRe_seq());
			pstmt.executeUpdate();
			
			sql="insert into n_board(num,name,passwd,subject,content,re_ref,re_lev,re_seq,readcount,date) values(?,?,?,?,?,?,?,?,?,?)";
			pstmt=con.prepareStatement(sql);
			pstmt.setInt(1, num);
			pstmt.setString(2, boardbean.getName());
			pstmt.setString(3, boardbean.getPasswd());
	...
	public int getCount(String search) {
		...
		try {
			con=getConnection();
			sql = "select count(*) from n_board where subject like ?";
			pstmt=con.prepareStatement(sql);
	...
	public List<BoardBean> getBoardList(int startRow, int pageSize, String search){
		...
		try {
			con=getConnection();
			sql="select * from n_board where subject like ? order by re_ref desc, re_seq asc limit ?,?";
			pstmt=con.prepareStatement(sql);
			pstmt.setString(1, "%"+search+"%");
	...
	public void insertComment(CommentBean cmtbean) {
		...
		try {
			con=getConnection();
			sql="insert into n_comment values(?,?,?,?,now())";
			pstmt=con.prepareStatement(sql);
	...
	public void deleteComment(int num, int cnum) {
		...
		try {
			con = getConnection();
			sql = "delete from n_comment where num = ? and cnum=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			pstmt.setInt(2, cnum);
			pstmt.executeUpdate();
...
public ArrayList<CommentBean> getComment(int num){
		...
		try {
			con=getConnection();
			String sql = "SELECT * FROM n_comment WHERE num = ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			rs = pstmt.executeQuery();
