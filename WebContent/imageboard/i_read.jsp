...
%>
<jsp:useBean id="cmtbean" class="i_board.CommentBean"/>
<body>
<div id="wrap">
<!-- 헤더들어가는 곳 -->
...
<!-- 		</tr> -->
		<tr>
			<td>이미지</td>
			<td colspan="3">
			<img height ="300" width="500" src="http://localhost:8088/MyProject2/temp/<%=FileName %>">
			</td>
		</tr>
	</table>
<%
...
%> 
	...
	%>	
		<input type="button" value="수정" class="btn" onclick="location.href='i_update.jsp?num=<%=num%>'">
		<input type="button" value="삭제" class="btn" onclick="location.href='i_delete.jsp?num=<%=num%>'">
	<%}%>	
<!-- 		<input type="button" value="답글" class="btn" -->
<%-- 		 onclick="location.href='i_reWrite.jsp?num=<%=num%>&re_ref=<%=Re_ref%>&re_lev=<%=Re_lev%>&re_seq=<%=Re_seq%>'"> --%>
	</div>
<%	
...
%>					<tr>
						...
						<%=new SimpleDateFormat("yyyy-MM-dd hh:mm:ss a").format(A.getDate())%>&nbsp;&nbsp;
...
<form action="i_contentAction.jsp?num=<%=num %>" method="post">
	<%
		if(session.getAttribute("id")!=null){		
	%>
	<tr>
