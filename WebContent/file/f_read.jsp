...
%>
<jsp:useBean id="cmtbean" class="f_board.CommentBean"/>
<body>
<div id="wrap">
<!-- 헤더들어가는 곳 -->
...
%> 
	...
	%>	
		<input type="button" value="수정" class="btn" onclick="location.href='f_update.jsp?num=<%=num%>'">
		<input type="button" value="삭제" class="btn" onclick="location.href='f_delete.jsp?num=<%=num%>'">
	<%} %>	
<!-- 		<input type="button" value="답글" class="btn" -->
<%-- 		 onclick="location.href='f_reWrite.jsp?num=<%=num%>&re_ref=<%=Re_ref%>&re_lev=<%=Re_lev%>&re_seq=<%=Re_seq%>'"> --%>
	</div>
<%	
...
<form action="f_contentAction.jsp?num=<%=num %>" method="post">
	<%
		if(session.getAttribute("id")!=null){		
	%>
	<tr>
